...
window.doRefresh = async function() {
  ...
  try {
    // Call the sync via a Supabase Edge Function (if set up)
    // For now: just reload data from Supabase
    // When Edge Function is ready: await fetch('/api/sync', { method: 'POST' })
    await loadData();
    updateSyncTimestamp();
    renderAll();
  } finally {
